...
protocol AuthPresenterProtocol: AnyObject {
    ...
    init(view: AuthViewProtocol, networkService: NetworkServiceProtocol)
...
final class AuthPresenter: AuthPresenterProtocol {
    ...
    required init(view: AuthViewProtocol, networkService: NetworkServiceProtocol) {
        self.view = view
        ...
        networkService.postRequestToLogin(uuid: "hello") { [weak self] response in
            ...
            case .success(let response):
                ...
                DispatchQueue.main.async {
                    self?.view?.presentHomeController(controller: ModuleBuilder.createHomeModule(response: response))
                }
